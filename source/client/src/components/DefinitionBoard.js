...
class DefinitionBoard extends React.Component {
  ...
  saveHandler() {
    ...
    if(notDefined === false){
      this.setState({kpiList: update(this.state.kpiList, {[this.state.kpiSelected] :{defined:{$set: this.kpiState.DEFINED}}})})
    }
    else{
      this.setState({kpiList: update(this.state.kpiList, {[this.state.kpiSelected] :{defined:{$set: this.kpiState.PARTIALLY_DEFINED}}})})
    }
  ...
  checkEmptyDefinition(){
    var disable = true
    for(var i of this.state.kpiList[this.state.kpiSelected].definitionField.values()){
      if(i!==""){
        disable = false
        break;
      }
        ...
        <div>
          {this.state.kpiSelected !== null ? (
            <div className={classes.gridContainer} >
              ...
              <Grid item xs={12}>
                <Button variant='contained' className={classes.button} disabled={this.checkEmptyDefinition()} onClick={this.saveHandler} color='primary'>SAVE</Button>
              </Grid>
            </div>
